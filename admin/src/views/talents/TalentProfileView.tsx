...
function renderFieldValue(field: CategoryField, value: any) {
...
export default function TalentProfileView({
  ...
  const sourceProfiles = profile?.source_profiles ?? [];
  const sourceCategoryIds = sourceProfiles.map((s) => s.category_id);

  const { data: sourceFieldsMap } = useQuery<Record<string, CategoryField[]>>({
    queryKey: ['ghost-source-fields', sourceCategoryIds],
    queryFn: async () => {
      const unique = Array.from(new Set(sourceCategoryIds));
      const results = await Promise.all(
        unique.map(async (catId) => {
          const { data } = await api.get(`/admin/categories/${catId}/fields`);
          return { catId, fields: (data.fields ?? data) as CategoryField[] };
        })
      );
      const map: Record<string, CategoryField[]> = {};
      for (const r of results) map[r.catId] = r.fields;
      return map;
    },
    enabled: isGhost && sourceCategoryIds.length > 0,
  });
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <div className="mt-1 flex items-center gap-1.5 text-sm text-gray-600">
            ...
            {(talentUser?.age != null || talentUser?.gender) ? (
              <span>
                {[
              ...
              <span className="text-gray-400 italic">Age &amp; gender not specified</span>
            )}
          </div>
        </div>
        ...
        <>
          <div className="rounded-xl border border-amber-200 bg-amber-50 p-4">
            <p className="text-sm text-amber-800">
              <span className="font-semibold">Combined Profile (Designer + Editor)</span>
              {' — '}data is sourced from the two underlying profiles shown below.
            </p>
          </div>

          {sourceProfiles.length === 0 && (
            <div className="rounded-xl border border-gray-200 bg-white p-6 text-sm text-gray-500">
              Source profiles are unavailable.
            </div>
          )}
...
function SourceProfileSection({
  ...
  const categories: { skill: string; level: number }[] = rawCategories
    ...
    .sort((a, b) => b.level - a.level);
  const categoriesLabel =
    source.category?.slug === 'designer' ? 'Categories and Skills' : 'Categories';

  const portfolioBySkill: Record<string, any[]> = {};
  for (const item of source.portfolio_items ?? []) {
    if (!portfolioBySkill[item.skill_name]) portfolioBySkill[item.skill_name] = [];
    portfolioBySkill[item.skill_name].push(item);
  }
  ...
  return (
    <div className="space-y-4">
          ...
          <dl className="divide-y divide-gray-100">
            {sortedFields.map((field) => (
              <div key={field.id} className="py-3 sm:flex sm:gap-4">
                <dt className="text-sm font-medium text-gray-500 sm:w-1/3">{field.field_label}</dt>
                <dd className="mt-1 text-sm text-gray-900 sm:mt-0 sm:w-2/3">
                  {renderFieldValue(field, source.field_data?.[field.field_key])}
                </dd>
              </div>
            ))}
          </dl>
      ...
      {categories.length > 0 && (
        <div className="rounded-xl border border-gray-200 bg-white p-6">
          <h3 className="mb-3 text-base font-semibold text-gray-900">{categoriesLabel}</h3>
          <div className="space-y-2">
            {categories.map((c) => (
              <div key={c.skill} className="flex items-center gap-3">
                <span className="w-40 text-sm font-medium text-gray-700">{c.skill}</span>
                ...
                <span className="w-8 text-center text-sm font-semibold text-indigo-600">{c.level}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      {skills.length > 0 && (
        <div className="rounded-xl border border-gray-200 bg-white p-6">
          <h3 className="mb-3 text-base font-semibold text-gray-900">Skill Sets</h3>
          <div className="space-y-2">
            {skills.map((s) => (
              <div key={s.skill} className="flex items-center gap-3">
                <span className="w-40 text-sm font-medium text-gray-700">{s.skill}</span>
      ...
      {Object.keys(portfolioBySkill).length > 0 && (
        <div className="rounded-xl border border-gray-200 bg-white p-6">
          <h3 className="mb-4 text-base font-semibold text-gray-900">Portfolio</h3>
          <div className="space-y-6">
            {Object.entries(portfolioBySkill).map(([skillName, items]) => (
              <div key={skillName}>
                <h4 className="mb-3 text-sm font-semibold text-gray-700">{skillName}</h4>
                <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                  {items.map((item: any) => (
                    <div key={item.id} className="rounded-lg border border-gray-200 p-3">
                      {item.file_type === 'image' && (
