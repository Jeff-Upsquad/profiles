...
import { WORK_TYPE_SEEKING_OPTIONS, GENDER_OPTIONS } from '@/constants/lead-form-options';
...
export default function SalesLeadForm() {
  ...
  useEffect(() => {
    axios
      .get('/api/leads/form-status/sales')
      .then((res) => {
  ...
  const validate = (): { ok: boolean; errs: Partial<Record<keyof FormValues, string>> } => {
    ...
    if (!form.industry_experience.trim()) errs.industry_experience = 'Industry experience is required';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const { data } = await axios.post('/api/leads/submit', {
        ...
        industry_experience: form.industry_experience.trim(),
      ...
      <div className="mx-auto w-full max-w-5xl px-5 py-6 sm:px-8 lg:px-10">
        ...
        <header className="saas-lift card-hero relative mt-3 overflow-hidden px-6 py-5 sm:mt-5 sm:px-10 sm:py-8 lg:px-12 lg:py-9">
          ...
          <div className="relative grid grid-cols-1 gap-4 lg:grid-cols-[1fr_320px] lg:items-end">
            <div>
              <span className="badge-prism">Talent · Sales Track</span>
              <h1 className="font-display-saas mt-2 text-xl font-bold leading-[1.15] text-canvas-900 sm:text-3xl sm:leading-[1.1] lg:text-[36px]">
                ...
                <span className="text-prism text-prism-animated">who love to close.</span>
              </h1>
              <p className="mt-1.5 max-w-xl text-sm leading-relaxed text-canvas-600 sm:mt-2">
                Tell us who you are and where you&rsquo;ve sold — we&rsquo;ll match you with Businesses,
                Brands, and Employers who need people who can grow revenue. Three chapters, about two minutes.
              </p>
            </div>

            <div className="surface-saas grid grid-cols-2 gap-3 px-4 py-3 sm:gap-4 sm:px-4 sm:py-3">
              <div>
                <p className="font-display-saas text-lg font-bold text-canvas-900 sm:text-2xl">03</p>
                <p className="mt-0.5 text-xs font-medium text-canvas-500 sm:mt-1">Chapters</p>
              </div>
              <div>
                <p className="font-display-saas text-lg font-bold text-canvas-900 sm:text-2xl">~2m</p>
        ...
        <form onSubmit={handleSubmit} noValidate className="pb-20">
          ...
          <Section
            index="03"
            title="Experience"
            description="What you do, and where you&rsquo;ve done it."
            delay={0.15}
          >
            ...
            <div id="field-industry_experience" className="field-saas">
              <Input
                label="Industry Experience"
                required
                placeholder="e.g. Real Estate, SaaS, FMCG, Banking"
                value={form.industry_experience}
                onChange={set('industry_experience')}
                error={errors.industry_experience}
              />
              <p className="mt-2 text-sm text-canvas-500">
                Which industries have you sold in? List the ones you know best.
              </p>
            </div>
          </Section>
