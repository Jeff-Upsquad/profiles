...
function AcceptedProfilesList({ categoryId }: { categoryId: string }) {
  ...
  if (isLoading) {
    return (
      <div className="overflow-hidden rounded-2xl border border-[#ECECEF] bg-white">
  ...
  if (!profiles || profiles.length === 0) {
    return (
      <div className="rounded-2xl border border-[#ECECEF] bg-white px-6 py-12 text-center">
        <div className="mx-auto mb-3 flex h-12 w-12 items-center justify-center rounded-2xl bg-[#F2EEFF]">
        ...
        <p className="max-w-sm mx-auto text-sm text-[#838383]">
  ...
  return (
    <li className={`stagger-${Math.min(index + 1, 6)}`}>
      <Link
        ...
        className="group flex items-center gap-4 px-5 py-4 transition-colors hover:bg-[#F8F9FA] sm:px-6"
      >
        {photo ? (
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-0.5 truncate font-[family-name:var(--font-inter)] text-xs text-[#A1A1AA]">
            {categoryName}
        ...
        <svg
          className="h-4 w-4 shrink-0 text-[#A1A1AA] transition-transform group-hover:translate-x-0.5 group-hover:text-[#6647F0]"
          fill="none"
          stroke="currentColor"
          viewBox="0 0 24 24"
          strokeWidth={2.25}
