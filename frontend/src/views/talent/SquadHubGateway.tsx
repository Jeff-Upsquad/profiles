...
// and knows whether a build has actually been published, so version handling
// stays in one place. Talents get the PARTNER app, not the business one.
const SQUADHUB_APK_URL = 'https://squadhub.in/partner-app';

// Not squadhub.in directly: this local page mints a one-time sign-in code with
// the talent's session and forwards them, so the web option lands them
// already signed in. See frontend/src/app/talent/squadhub/launch/page.tsx.
const SQUADHUB_WEB_URL = '/talent/squadhub/launch';

/**
 * The talent SquadHub gateway — the twin of the business one, shown once the
 * talent is assigned to their first client. Three ways in: the Android partner
 * app, iOS (not yet), and the web app.
 *
 * The web option signs them in automatically, creating their SquadHub partner
 * account on the spot if they don't have one. The role they land with follows
 * the card that assigned them — designer, video editor, accountant and so on.
 */
export default function SquadHubGateway() {
  const router = useRouter();
  const { hasAssignedCard, isLoading } = useTalentHasAssignedCard();

  // Guard the route itself, not just the nav entry — this page is meaningless
  // before the talent is actually working on a card.
  useEffect(() => {
    ...
    <div className="space-y-6">
        ...
        <div className="hero-content flex flex-col items-start gap-4 sm:flex-row sm:items-center">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="mt-1.5 max-w-xl font-[family-name:var(--font-jakarta)] text-sm text-[#525252] stagger-3">
              You&rsquo;re assigned to a client — SquadHub is where the work happens. Pick up
              tasks, chat with the team, and track your deliverables day to day.
            </p>
          </div>
        </div>
      </section>
      ...
      <div className="flex items-start gap-3 rounded-2xl border border-[#E7E7EA] bg-[#FFFAC2]/40 px-5 py-4">
        ...
        <p className="font-[family-name:var(--font-inter)] text-[13px] leading-relaxed text-[#525252]">
          Opening SquadHub on the web{' '}
          <span className="font-semibold text-[#0a0a0a]">signs you straight in</span> — no password
          needed, and your account is set up for you the first time. In the app, use the same email
      ...
      <div className="space-y-3">
        <OptionCard
          ...
          body="Install the SquadHub Partner app on your Android phone."
