...
export default function EducationPicker({ value, onChange }: EducationPickerProps) {
  const addEntry = () => {
    onChange([...value, { from_year: '', from_month: '', to_year: '', to_month: '', course_name: '', institution: '' }]);
  };

  const updateEntry = (index: number, field: keyof EducationEntry, val: string) => {
    const next = value.map((entry, i) => {
      if (i !== index) return entry;
      if (field === 'course_name' || field === 'institution') {
    ...
    <div>
      {value.length > 0 && (
        <div className="space-y-4">
          {value.map((entry, i) => (
            <div key={i} className="relative rounded-xl border border-[#E8E5DE] bg-[#F7F6F3] p-5">
              <button
                type="button"
              ...
              <div className="space-y-4">
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  <Input
                    ...
                    onChange={(e) => updateEntry(i, 'course_name', e.target.value)}
                    placeholder="e.g. B.Com, Digital Marketing"
                  />
                  <Input
                    label="Institution / Where Learned"
                    value={entry.institution}
                    onChange={(e) => updateEntry(i, 'institution', e.target.value)}
                    placeholder="e.g. Delhi University, Coursera"
                  />
                </div>
              </div>
            </div>
      ...
      <button
        type="button"
        onClick={addEntry}
        className="mt-4 inline-flex items-center gap-1.5 rounded-lg px-3 py-2 text-sm font-medium text-[#0a0a0a] hover:bg-[#F7F6F3] transition-colors"
      >
        <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
