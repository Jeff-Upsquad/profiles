...
const PLAN_OPTIONS: {
  ...
  { name: 'Basic', dailyHours: 2, weeklyHours: 10, monthlyHours: 40, pct: '25%', capacity: 'Quarter of a full-time employee', perDay: '2 hours per day', perWeek: '10 hours per week', bestFor: 'Active brands', tagline: 'Our standard and most affordable plan.' },
  { name: 'Plus', dailyHours: 4, weeklyHours: 20, monthlyHours: 80, pct: '50%', capacity: 'Half employee capacity', perDay: '4 hours per day', perWeek: '20 hours per week', bestFor: 'High-volume teams & agencies', tagline: 'Get your tasks completed faster with elevated priority.', recommended: true },
  { name: 'Pro', dailyHours: 6, weeklyHours: 30, monthlyHours: 120, pct: '80%', capacity: 'Nearly full-time employee', perDay: '6 hours per day', perWeek: '30 hours per week', bestFor: 'Growing businesses', tagline: 'Highest speed and fastest response time.' },
...
export default function DesignerBriefForm({
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (isServiceRequest) {
      ...
      if (incompleteRole) {
        validationErrors.push({
          message: 'Select a service type for each service in Requirement.',
          targetId: 'brief-requirement',
        });
      }
    }
        ...
        validationErrors.push({
          message:
            missingPlan.length === 1
              ? 'Select a weekly plan in Plan, levels & budget.'
              : 'Select a weekly plan for each role in Plan, levels & budget.',
          targetId: 'brief-plan',
        });
      }
    }
  ...
  if (submitted) {
    return (
      <div className="connect-bg flex min-h-full items-center justify-center px-4 py-12">
        ...
        {step === 2 && (
          <form onSubmit={handleSubmit} noValidate className="space-y-5 pb-8">
            ...
            <Section
              ...
              eyebrow="Requirement"
              title="What should your creative talent own?"
              hint="Be specific about the outcome, recurring work, and day-to-day responsibility. Add a written brief, a voice note, or both."
            >
              <div className="space-y-4">
                {product === 'assignment' && (
                  <div>
                ...
                <Field label="Describe your requirement" optional hint="Explain the kind of work you're looking to get done.">
                  <textarea
                    rows={4}
                    ...
                    placeholder={isServiceRequest
                      ...
                      : 'e.g. Own our weekly social media creatives, one brand video a month, and occasional pitch decks…'}
                    className="connect-input resize-none"
                  />
                </Field>
            ...
            {/* Section: Subscription — plan + levels + per-level budgets. */}
            <Section
              id="brief-plan"
                          ...
                          {PLAN_OPTIONS.map((p) => {
                            ...
                            return (
                              <button
                                ...
                                type="button"
                                aria-pressed={on}
                                onClick={() => updateRoleReq(opt.slug, 'plan', p.name)}
  ...
  async function start() {
    ...
    try {
      ...
      if (name === 'NotAllowedError' || name === 'SecurityError') {
        setErr('Microphone permission is off. Allow it in your browser settings — or use “Upload audio” to record with your phone instead.');
      } else if (name === 'NotFoundError' || name === 'OverconstrainedError') {
        setErr('No microphone found. Use “Upload audio” to attach a recording instead.');
      } else {
        setErr('Couldn’t start recording here. Use “Upload audio” to record with your phone instead.');
      }
    }
  }
  function stop() {
