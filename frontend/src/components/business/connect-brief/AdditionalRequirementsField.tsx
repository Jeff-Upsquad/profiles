...
// Mirrors squadhub's shared catalog. Businesses pick chips or add their own; the
// selection is forwarded to squadhub as role_requirements.<role>.additional_
// requirements. Descriptive only — never used to match talent during broadcast.
...
const CATALOG: Record<string, Group[]> = {
...
function catalogFor(slug: string): Group[] {
  return CATALOG[slug] || AR_DESIGNER;
}
 ...
 * Single visible toggle; when on, shows the category catalog chips plus an
    ...
    <div className="arq-card">
      ...
      {open && (
        <div className="arq-reveal">
          {roles.map((role) => {
            const groups = catalogFor(role.slug);
            return (
