...
export async function resolveCardLink(req: Request, res: Response, next: NextFunction) {
  try {
    const result = await businessService.resolveCardPortalPath(
      req.user!.id,
      req.params.cardId as string,
    );
    res.json(result);
...
export async function unselectCardRecipient(req: Request, res: Response, next: NextFunction) {
  try {
    const result = await businessService.businessUnselectRecipient(
      req.user!.id,
      req.params.cardId as string,
    );
    res.json(result);
