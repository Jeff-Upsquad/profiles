...
export async function signupTalentApplication(req: Request, res: Response, next: NextFunction) {
  try {
    const result = await authService.signupTalentApplication(req.body);
    res.status(201).json(result);
  } catch (err) {
    next(err);
  }
